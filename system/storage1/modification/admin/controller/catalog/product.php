  ...
  // multi image uploader
	public function muimage() {
		...
		if (isset($this->request->get['image'])) {
			$image = $this->model_tool_image->resize($this->request->get['image'], 100, 100);
      echo $image;
		}
	}
	// multi image uploader	
	...
	protected function getList() {
		...
		$data['products'] = array();
			...
			$data['product_combinations_enabled'] = false;
            if ($this->config->get('config_opt_comb_status')){
                ...
                $data['product_combinations_enabled'] = true;
                $data['button_combined_options'] = $this->language->get('button_combined_options');
...
if ($this->config->get('config_opt_comb_status')){
                $combinations = $this->model_extension_module_options_combinations->getDisplayOptionsCombinations( $result['product_id'], $product_specials, false, $url);
            } else {
                $combinations = [];
            }
...
'combinations' => $combinations,
		...
		$data['user_token'] = $this->session->data['user_token'];
				...
				$data['options_combinations_js_errors'] = isset($this->error['options_combinations_js_errors']) ? $this->error['options_combinations_js_errors'] : '';
		...
		$data['footer'] = $this->load->controller('common/footer');
				...
				if($this->config->get('config_opt_comb_status') && __FUNCTION__ == 'getForm'){
				    $this->load->model('extension/module/options_combinations');
				    $view_path = $this->model_extension_module_options_combinations->format_controller('extension/module/options_combinations/getOptionsForm');
					$data['combined_options']= $this->load->controller($view_path,$data);
				} else {
	...
	protected function getForm() {
		...
		$data['text_intab'] = $this->language->get('text_intab');
		$data['text_outtab'] = $this->language->get('text_outtab');
		$data['entry_displayvideo'] = $this->language->get('entry_displayvideo');
		$data['entry_videotitle'] = $this->language->get('entry_videotitle');
		$this->model_catalog_product->CreateProductvideotable();
		///XML
		...
		///XML
		if (isset($this->request->post['product_video_display'])) {
			$data['product_video_display'] = $this->request->post['product_video_display'];
		} elseif (!empty($product_info)) {
			$data['product_video_display'] = $product_info['product_video_display'];
		} else {
			$data['product_video_display'] = 0;
		}
		///XML
		...
		// Categories
				...
				//faqgroup start 
	    $data['entry_faqgroup'] = $this->language->get('entry_faqgroup');
				
				if (isset($this->request->post['faqgroup_id'])) {
				$data['faqgroup_id'] = $this->request->post['faqgroup_id'];
			} elseif (!empty($product_info)) {
				$data['faqgroup_id'] = $product_info['faqgroup_id'];
			} else {
				$data['faqgroup_id'] = '';
			}
			...
			if (!empty($product_info)) {
				$faqgroup_info = $this->model_extension_faqgroup->getfaqgroup($product_info['faqgroup_id']);
			} elseif(isset($this->request->post['faqgroup_id'])){
				$faqgroup_info = $this->model_extension_faqgroup->getfaqgroup($this->request->post['faqgroup_id']);
			}else{
				$faqgroup_info = array();
			}
			
			if($faqgroup_info) {
				$data['faqgroup_faqs'] =  $faqgroup_info['title'];
			}else{
				$data['faqgroup_faqs'] = '';
			}
		//faqgroup end
        ...
        $data['img_dir']=$this->config->get('multiimageuploader_folder');
        // multi image uploader
		...
		$data['product_videos'] = array();
			...
			foreach($product_videos as $product_video){
				if (isset($product_video['image']) && is_file(DIR_IMAGE .$product_video['image'])) {
					$productvideo_image = $this->model_tool_image->resize($product_video['image'], 100, 100);
				} else {
					$productvideo_image = $this->model_tool_image->resize('no_image.png', 100, 100);
				}
				
				$data['product_videos'][] = array(
					'status' => $product_video['status'],
					'video_url' => $product_video['video_url'],
					'thumb' => $productvideo_image,
					'image' => $product_video['image'],
					'video_description' => $product_video['video_description'],
				);
			}
		...
		if ($this->error && !isset($this->error['warning'])) {
			$this->error['warning'] = $this->language->get('error_warning');
				...
				if($this->config->get('config_opt_comb_status')){
					$this->load->model('extension/module/options_combinations');
					$tempError = array();
					$tempError = array_merge($tempError, $this->model_extension_module_options_combinations->validateForm($this));
					$this->error = !empty($tempError) ? $tempError : $this->error;
				} 
		return !$this->error;
	}
    ...
    private function getCombinedOptions2( $product_id) {

	    $opc = new \optionscombinations\OptionsCombinationsLib($this->registry);

        $result = $this->model_extension_module_options_combinations->getProductOptionsCombinations( $product_id)->rows;

        foreach ($result as &$combination) {
           $combination['options'] = $opc->getCombinationOptionsMapped($combination['id']);
        }
        return $result;
    }
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model'])) {
			...
			foreach ($results as $result) {
                ...
                'combined_options2' => $this->config->get('config_opt_comb_status') ? $this->getCombinedOptions2( $result['product_id']) : false,
				'combined_options' => $this->config->get('config_opt_comb_status') ? $this->model_extension_module_options_combinations->getProductOptions( $result['product_id']) : false,
