...
class ControllerCatalogOption extends Controller {
	...
	protected function validateDelete() {
        ...
        $this->load->language('extension/module/options_combinations_errors');
        $optionscombinations_lib = new OptionsCombinationsLib($this->registry);
		//$optionscombinations_lib = new \optionscombinations\OptionsCombinations($this->registry);
            

		foreach ($this->request->post['selected'] as $option_id) {
			$product_total = $this->model_catalog_product->getTotalProductsByOptionId($option_id);

			$combination_total = count($optionscombinations_lib->getCombinations(['option_id' => $option_id]));
			if ($combination_total > 0)
                $this->error['warning'] = sprintf($this->language->get('error_removing_option'), $combination_total);
		...
		if (isset($this->request->get['filter_name'])) {
			...
			foreach ($options as $option) {
				...
				if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'image') {
                    ...
                    if (!isset($this->request->get['default_sort']))
