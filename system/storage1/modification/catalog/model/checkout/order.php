...
class ModelCheckoutOrder extends Model {
  public function addOrder($data) {
      ...
      foreach ($data['products'] as $product) {
        ...
        $order_product_id = $this->db->getLastId();
            ...
            if (isset($product['product_url'])) {
                $this->db->query("UPDATE " . DB_PREFIX . "order_product SET product_url = '" . $this->db->escape($product['product_url']) . "', details = '" . $this->db->escape($product['details']) . "' WHERE model = 'CUSTOM_ORDER' AND order_product_id = '" . (int)$order_product_id . "'");
            }
            // Custom Search - / End
    ...
    $this->db->query("UPDATE `" . DB_PREFIX . "order` SET tntorder_id = '" . $tntorder_id . "' WHERE order_id = '" . (int)$order_id . "'");
    //tnt
    ...
    if ($order_info) {
      ...
      if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
        ...
        foreach ($order_products as $order_product) {
                    ...
                    $quantity_sql = "quantity - " . (int)$order_product['quantity'];

                    if ($this->config->get('config_opt_comb_keep_stock_at_zero'))
                        $quantity_sql = "GREATEST(0, ".$quantity_sql.")";

                    $this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = (".$quantity_sql.") WHERE product_id = '" . (int)$order_product['product_id'] . "' AND subtract = '1'");

					if (isset($order_product['combination_id']) && $order_product['combination_id']){
						$this->load->model('extension/module/options_combinations');
						$this->model_extension_module_options_combinations->reduceCombinationQuantity((int)$order_product['combination_id'], (int)$order_product['quantity']);
					} 
      ...
      if (in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && !in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
        ...
        foreach($order_products as $order_product) {
          $this->db->query("UPDATE `" . DB_PREFIX . "product` SET quantity = (quantity + " . (int)$order_product['quantity'] . ") WHERE product_id = '" . (int)$order_product['product_id'] . "' AND subtract = '1'");
				...
				if (isset($order_product['combination_id']) && $order_product['combination_id']){
					$this->load->model('extension/module/options_combinations');
					$this->model_extension_module_options_combinations->increaseCombinationQuantity((int)$order_product['combination_id'], (int)$order_product['quantity']);
				} 
