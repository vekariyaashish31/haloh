	...
	public function index() {
		...
		if ($product_info) {
			...
			$this->load->model('catalog/review');
				...
				//faqgroup start
			$data['tab_faqgroup'] = $this->language->get('tab_faqgroup');
			...
			$data['faqs'] = array();
			if (!empty($product_info['faqgroup_id'])) {
				$faqs = $this->model_catalog_product->getFaqgroupDesc($product_info['faqgroup_id']);
			  
				foreach ($faqs as $faqvalue) {
					$data['faqs'][] = array(
						'faqgroup_id' 						=> $faqvalue['faqgroup_id'],
						'faq_id' 						=> $faqvalue['faq_id'],
						'name' 						=> $faqvalue['name'],
						'faqdescription'  => strip_tags(html_entity_decode($faqvalue['description'], ENT_QUOTES, 'UTF-8'))
					);
				}
			}
			//faqgroup end
			...
			if ($this->config->get('config_tax')) {
				...
				$file = 'library/optionscombinations/modify_product_prices.php';
			...
			foreach($product_videos as $product_video){
				...
				if ($product_video['image']) {
					$image = $this->model_tool_image->resize($product_video['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_height'));
				} else {
					$image = 'image/no_image.png';
				}
				
				$data['product_videos'][] = array(
					'product_video_id' => $product_video['product_video_id'],
			...
			//tnt
			if(!empty(count($data['product_videos'][0]))){
				$data['tntproduct_video_id'] 	= $data['product_videos'][0]['product_video_id'];
    			$data['tntstatus'] 				= $data['product_videos'][0]['status'];
    			$data['tntvideo_url'] 			= $data['product_videos'][0]['video_url'];
    			$data['tntimage'] 				= $data['product_videos'][0]['image'];
    			$data['tnttitle'] 				= $data['product_videos'][0]['title'];
			}
			//tnt
			/*Xml*/
			...
			foreach ($results as $result) {
				...
				$data['products'][] = array(
					...
					'combination_as_product' => !empty($result['combination_as_product']) ? $result['combination_as_product'] : array(),
			...
			$this->model_catalog_product->updateViewed($this->request->get['product_id']);
				...
				$file = 'library/optionscombinations/modify_product_data.php';
				$path_modification = DIR_MODIFICATION.'system/'.$file;
				$path_temp = DIR_SYSTEM.$file;
				require_once (is_file($path_modification) ? $path_modification : $path_temp); 
