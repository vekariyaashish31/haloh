...
class ControllerExtensionModuleLatest extends Controller {
	public function index($setting) {
		...
		if ($results) {
			...
			$data['doublerow'] = 1;
			$totalpro = (int)count($results);

			$desktopget = floor($totalpro / 5);
			$desktopaap = $setting['tntpcrow'];
			$data['desktop'] = $setting['tntpcrow'];
			if($desktopaap >= $desktopget){
				$data['desktop'] =  $desktopget;
			} else{
				$data['desktop'] = $setting['tntpcrow'];
			}

			$tabletget = floor($totalpro / 4);
			$tabletaap = $setting['tnttbrow'];
			$data['tablet'] = $setting['tnttbrow'];
			if($tabletaap >= $tabletget){
				$data['tablet'] =  $tabletget;
			} else{
				$data['tablet'] = $setting['tnttbrow'];
			}

			$mobileget = floor($totalpro / 2);
			$mobileaap = $setting['tntmbrow'];
			$data['mobile'] = $setting['tntmbrow'];
			if($mobileaap >= $mobileget){
				$data['mobile'] =  $mobileget;
			} else{
				$data['mobile'] = $setting['tntmbrow'];
			}
