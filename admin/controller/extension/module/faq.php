...
class ControllerExtensionModuleFaq extends Controller {
	private $error = array();
	  ...
	  public function index(){
		...
		$data['help_faqgroup'] = $this->language->get('help_faqgroup');
		...
		if (!empty($this->request->post['faqgroup'])) {
			$faqgroups = $this->request->post['faqgroup'];
		} elseif (!empty($module_info['faqgroup'])) {
			$faqgroups = $module_info['faqgroup'];
		} else {
			$faqgroups = array();
		}

		foreach($faqgroups as $faqgroup_id){
		$faqgroup_info = $this->model_extension_faqgroup->getfaqgroup2($faqgroup_id);
		foreach($faqgroup_info as $test)
			{
					$data['faqgroups'][] = array(
					'faqgroup_id' => $test['faqgroup_id'],
					'name'        => $test['title']
				);
			}
		}
