...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['shipping_code'])) {
			...
			$data['shipping_code'] = $customer_info['shipping_code'];
		} else {
			$data['shipping_code'] = '';
		}
	...
	protected function validateForm() {
		...
		if (utf8_strlen($this->request->post['shipping_code']) <= 0) {
			$this->error['shipping_code'] = 'Shipping Code Required!';
		}

		$customer_info = $this->model_customer_customer->getCustomerByShippingCode($this->request->post['shipping_code']);

		if (!isset($this->request->get['customer_id'])) {
			if ($customer_info) {
				$this->error['warning'] = 'Warning: Shipping Code already used!';
			}
		} else {
